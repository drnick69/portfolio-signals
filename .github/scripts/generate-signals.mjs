...
const cyclicalWarning = isCyclical ? `CRITICAL — CYCLICAL VALUATION RULES FOR ${h.symbol}: ${h.symbol} is a CYCLICAL business (archetype: ${h.archetype}). Trailing P/E must be interpreted INVERSELY: • HIGH trailing P/E (>50x) = earnings are at TROUGH = this is a BUY signal, NOT expensive • LOW trailing P/E (<10x) = earnings are at PEAK = cycle rollover risk = TRIM signal • "Buy cyclicals when the P/E looks terrible, sell when it looks cheap." — Peter Lynch • The deterministic engine has already applied inverted PE scoring. Your qualitative score should NOT penalize high trailing P/E for this holding. Instead, consider whether the earnings trough is deepening or recovering.` : “”;
...
1. GAS VOLUMES + LNG BUILDOUT (medium-to-long-term earnings trajectory): This is YOUR primary value-add. ENB’s gas transmission is ~25% of EBITDA. Natural gas prices are a LEADING INDICATOR for throughput volumes, not a direct revenue driver. LNG export buildout (LNG Canada, US Gulf Coast expansion) creates structural demand for ENB’s pipeline capacity. Henry Hub price levels affect drilling activity which affects volumes.
1. CRUDE THROUGHPUT (Mainline economics): WCS-WTI spread, Canadian crude production growth, Trans Mountain dynamics. ENB’s Liquids Pipelines are ~55% of EBITDA.
...
1. RISK APPETITE: ETH is more sensitive to VIX, HY OAS, and real rates than BTC. In risk-off, ETH drops 1.3-1.5x what BTC drops. The engine scores this with enhanced macro weights.
1. ETH/BTC RATIO (ALT-SEASON): When capital rotates from BTC into alts, ETH outperforms (“alt season”). When BTC dominance rises, ETH underperforms. The engine computes this as ETHA/IBIT daily performance spread. Your job: assess whether a rotation is starting, peaking, or fading.
...
1. POLITICAL RISK: THIS IS YOUR #1 VALUE-ADD. Lula government interference is the dominant idiosyncratic risk. Watch for: CEO changes, fuel pricing formula violations, forced capex into uneconomic refining, dividend policy pressure, strategic plan deviations. A single headline can move PBR.A 5-10%.
1. BRL/USD: Strong BRL lifts the ADR, weak BRL drags it. The engine scores BRL level. Your job: assess Banxico/BCB policy, fiscal trajectory, carry trade dynamics.
...
1. CORN/AG PRICES — When corn is high ($6-7+/bu), farmers buy more fertilizer. When corn is low ($4/bu), they cut applications. The engine scores CORN ETF as a proxy.
1. SEASONAL CYCLES — The engine applies a seasonal modifier: spring planting (Mar-May) = peak demand bias, fall (Sep-Nov) = LatAm planting pulse, winter (Dec-Feb) = weakest period.
1. BRL/USD — Mild overlay. Mosaic Fertilizantes (Brazil operations) is significant.
...
1. STRUCTURAL TAILWINDS — multi-year compounding drivers (THIS IS YOUR PRIMARY VALUE-ADD):
...
1. BACKLOG GROWTH: Sale-of-gas + on-site project pipeline, reported quarterly. Leading indicator — earnings lag 2-4 quarters. Engine scores YoY growth.
...
1. AI CYCLE PHASE — secular growth at risk of expectation reset:
